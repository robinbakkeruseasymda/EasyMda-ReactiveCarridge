...
public class CreationSupport
{
  ...
  public static synchronized CreationSupport create(String lineSeparator)
  {
    nl = lineSeparator;
    CreationSupport result = new CreationSupport();
    nl = null;
    return result;
  }
  ...
  protected final String TEXT_2 = NL + NL + "import akka.actor.{ActorContext, ActorRef, Props}" + NL + "" + NL + "trait CreationSupport {" + NL + "  def getChild(name:String):Option[ActorRef]" + NL + "  def createChild(props:Props, name:String):ActorRef" + NL + "  def getOrCreateChild(props:Props, name:String):ActorRef = getChild(name).getOrElse(createChild(props, name))" + NL + "}" + NL + "" + NL + "trait ActorContextCreationSupport extends CreationSupport {" + NL + "  def context:ActorContext" + NL + "" + NL + "  def getChild(name:String):Option[ActorRef] = context.child(name)" + NL + "  def createChild(props:Props, name:String):ActorRef = context.actorOf(props, name)" + NL + "}";
